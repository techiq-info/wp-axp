...
if ( ! class_exists( 'EnableAccessibilityPlugin' ) ) :
	...
	class EnableAccessibilityPlugin {
		...
		public function __construct() {
			...
			// schedule cron only if we have a license key set
			if ( get_option( 'enable-accessibility' ) ) {
				$this->schedule_cron();
			}
		}
		...
		private function schedule_cron() {
			if ( ! wp_next_scheduled( 'enable_update_license_data' ) ) {
				wp_schedule_event( time(), 'daily', 'enable_update_license_data' );
			}
		}
		...
		public function enqueue_scripts() {
			...
			if ( $license_data && ! empty( $license_data->website->js ) ) {
				$js_url = $license_data->website->js;
			} else {
				$js_url = "https://cdn.{$this->enable_hosts[1]}/licenses/init.js";
			}
		...
		private function _get_license_data( $lkey ) {
			...
			foreach( $this->enable_hosts as $host ) {
				$response     = wp_safe_remote_get( "https://my.{$host}/modules/addons/goenable/api.php?litk=" . urlencode( $lkey ) . "&r=" . urlencode( site_url() ), array(
					'user-agent' => 'enable-accessibility/' . ENABLE_ACCESSIBILITY_VERSION,
					'sslverify' => false,
				) );
