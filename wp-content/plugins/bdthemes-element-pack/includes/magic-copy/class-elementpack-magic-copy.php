...
class ElementPack_Magic_Copy {
    ...
    function __construct() {
        add_action('wp_ajax_ep_elementor_import_magic_copy_assets_files', array($this, 'ajax_import_data'));
        add_action( 'elementor/editor/after_enqueue_scripts', [ $this, 'magic_copy_enqueue' ] );
    }

    public function magic_copy_enqueue(){
        wp_enqueue_script( 'bdt-magic-copy-storage', BDTEP_URL . 'includes/magic-copy/assets/xdLocalStorage.min.js', array(), BDTEP_VER );
        wp_enqueue_script( 'bdt-magic-copy-scripts', BDTEP_URL . 'includes/magic-copy/assets/element-pack-magic-copy.js', array( 'jquery','bdt-magic-copy-storage','elementor-editor'), BDTEP_VER, true);
        wp_localize_script(
